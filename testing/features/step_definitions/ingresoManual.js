...
Given('que la API del backend esta activa y lista para recibir peticiones', async function () {
    try {
        ...
        throw new Error(`No se pudo conectar con la API del backend en ${BACKEND_URL}: ${error.message}`);
    }
});
...
Given('que el usuario autenticado desea registrar un ingreso manual', function () {
    assert.ok(
        this.usuarioAutenticado,
